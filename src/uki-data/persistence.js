...
(function () {
    ...
    persistence.schemaSync = function (tx, callback) {
      var args = uki.argspec.getArgs(arguments, [
          { name: "tx", optional: true, check: isTransaction },
          { name: "callback", optional: true, check: uki.argspec.isCallback(), defaultValue: function(){} }
        ]);
      tx = args.tx;
    ...
    function getEntity(entityName) {
        ...
        for ( var field in meta.fields) {
          (function () {
              if (meta.fields.hasOwnProperty(field)) {
                ...
                //   return that._data[f];
                // });
                that[field] = function(val) {
                    if (val === undefined) return that._data[f];
                    var oldValue = that._data[f];
                    that._data[f] = val;
                    that._dirtyProperties[f] = oldValue;
                    that.triggerEvent('set', that, f, val);
      ...
      Entity.prototype.fetch = function(tx, rel, callback) {
        var args = uki.argspec.getArgs(arguments, [
            { name: 'tx', optional: true, check: isTransaction },
            { name: 'rel', optional: false, check: uki.argspec.hasType('string') },
            { name: 'callback', optional: false, check: uki.argspec.isCallback() },
          ]);
        tx = args.tx;
        rel = args.rel;
        callback = args.callback;
      ...
      Entity.load = function(session, tx, id, callback) {
        var args = uki.argspec.getArgs(arguments, [
            { name: 'session', optional: true, check: isSession, defaultValue: persistence },
            { name: 'tx', optional: true, check: isTransaction },
            { name: 'id', optional: false, check: uki.argspec.hasType('string') },
            { name: 'callback', optional: true, check: uki.argspec.isCallback(), defaultValue: function(){} }
          ]);
      ...
      Entity.findBy = function(session, tx, property, value, callback) {
        var args = uki.argspec.getArgs(arguments, [
            { name: 'session', optional: true, check: isSession, defaultValue: persistence },
            { name: 'tx', optional: true, check: isTransaction },
            { name: 'property', optional: false, check: uki.argspec.hasType('string') },
    ...
    persistence.dump = function(tx, entities, callback) {
      var args = uki.argspec.getArgs(arguments, [
          ...
          { name: 'callback', optional: false, check: uki.argspec.isCallback(), defaultValue: function(){} }
        ]);
      tx = args.tx;
    ...
    persistence.dumpToJson = function(tx, entities, callback) {
      this.dump(tx, entities, function(obj) {
          callback(uki.stringifyJSON(obj));
        });
    };
    ...
    persistence.loadFromJson = function(tx, jsonDump, callback) {
      this.load(tx, uki.parseJSON(jsonDump), callback);
    };
    ...
    DbQueryCollection.prototype.each = function (tx, eachFn) {
      var args = uki.argspec.getArgs(arguments, [
          { name: 'tx', optional: true, check: isTransaction },
          { name: 'eachFn', optional: true, check: uki.argspec.isCallback() }
        ]);
      tx = args.tx;
      eachFn = args.eachFn;
    ...
    DbQueryCollection.prototype.one = function (tx, oneFn) {
      var args = uki.argspec.getArgs(arguments, [
          { name: 'tx', optional: true, check: isTransaction },
          { name: 'oneFn', optional: false, check: uki.argspec.isCallback() }
        ]);
      tx = args.tx;
      oneFn = args.oneFn;
    ...
    LocalQueryCollection.prototype.count = function(tx, callback) {
      var args = uki.argspec.getArgs(arguments, [
          { name: 'tx', optional: true, check: isTransaction },
          { name: 'callback', optional: true, check: uki.argspec.isCallback() }
        ]);
      tx = args.tx;
      callback = args.callback;
